...
date: 2018-07-17
tags: [OpenVPN]
excerpt: Virtual Private Network (VPN) adalah sesuatu yang menghubungkan dua jaringan berbeda melalui jaringan publik (*public network*) sehingga pengguna seolah-olah berada dalam jaringan yang sama (*virtual*)...
---
...
```bash
openvpn --ifconfig 10.8.0.2 10.8.0.1 --dev tun --remote x.x.x.x --redirect-gateway def1 # x.x.x.x adalah IP server
```
...
:::note
Bila mengaktifkan `remote-cert-tls server|client` di konfigurasi OpenVPN untuk mencegah serangan *Man-In-The-Middle* (MITM), saya menemukan bahwa sertifikat yang dihasilkan akan gagal saat dipakai dengan pesan kesalahan seperti *unsupported certificate purpose* .  Sebagai solusinya, saya menggunakan perintah `./easyrsa gen-req` dan `./easyrsa sign-req server|client` untuk menghasilan sertifikat secara terpisah.  
:::
