...
date: 2022-03-10
tags: [Network Protocol, GNS3]
excerpt: Bila *firewall* melindungi dari serangan dari pihak eksternal, bagaimana dengan serangan dari dalam?  Sebagai contoh, bila ada penyusup yang masuk ke dalam gedung dan mencolokkan kabel LAN ke perangkat miliknya, ia akan mendapatkan akses internal sama seperti karyawan lain di gedung tersebut...
...
```shell
tar xvf PacketFence-ZEN-v11.2.0.ova
qemu-img convert -O qcow2 PacketFence-ZEN-v11.2.0-disk1.vmdk PacketFence-ZEN-v11.2.0-disk1.qcow2
```
...
![Rancangan Jaringan Di GNS3](../../../assets/gambar_20220310_001.png)
...
```shell {"Switch>": 1} {"Switch#": 2,14} {"Switch(config)#": 3,5,9,13} {"Switch(config-vlan)#": 4} {"Switch(config-if-range)#": 6-8} {"Switch(config-if)": 10-12}
       enable
       configure terminal
               vlan 44
                    exit
...
```shell {"Switch>": 1} {"Switch#": 2} {"Switch(config)#": 3-5,10,13-19,34} {"Switch(config-radius-server)#": 6-9} {"Switch(config-sg-radius)#":10-12} {"Switch(config-if-range)#": 20-33}
       enable
       configure terminal
               dot1x system-auth-control
               aaa new-model
               radius server packetfence
                             address ipv4 192.168.122.246 auth-port 1812 acct-port 1813
                             timeout 2
                             key mysecret
                             exit
...
![Wired 802.1X Di Windows 10](../../../assets/gambar_20220310_002.png)
...
![Komunikasi Antara Supplicant dan Authenticator](../../../assets/gambar_20220310_003.png)
...
![Komunikasi Antara Authenticator dan Authentication Server](../../../assets/gambar_20220310_004.png)
...
![Statistik RADIUS di PacketFence](../../../assets/gambar_20220310_005.png)
...
![Auditing Di PacketFence](../../../assets/gambar_20220310_006.png)