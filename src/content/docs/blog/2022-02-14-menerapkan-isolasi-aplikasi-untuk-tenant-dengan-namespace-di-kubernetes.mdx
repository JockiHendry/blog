...
date: 2022-02-14
tags: [Kubernetes]
excerpt: Pada suatu hari, anggap saja ada dua perusahaan fiktif dengan nama `perusahaan1` dan `perusahaan2` yang ingin memakai aplikasi latihan-k8s. Mereka selanjutnya akan disebut sebagai *tenant*.  Setiap *tenant* akan mendaftarkan beberapa *user* dengan hak akses berbeda.  Tentu saja mereka juga tidak ingin data mereka diakses oleh *tenant* lain...
...
```shell
./create-tenant.sh perusahaan1
./create-tenant.sh perusahaan2
```
...
![Dua Tenant Yang Berbeda](../../../assets/gambar_20220214_001.png)
...
![Keycloak Di Dua Tenant Yang Berbeda](../../../assets/gambar_20220214_002.png)
...
```shell
find . -type d -name "tenant-*" -print -execdir git -C $PWD/{} log -n1 --oneline \;
## Output:
## ./tenant-perusahaan2
...
```shell
cd tenant-perusahaan1
kpt fn eval --image gcr.io/kpt-fn/search-replace:v0.2.0 -- by-path='spec.replicas' put-value=10
kpt live apply
git add .
git commit -m "Increase number of pods"
```
