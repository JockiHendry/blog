...
date: 2022-02-26
tags: [Network Protocol, Ubuntu]
excerpt: Paket jaringan adalah unit data terkecil yang dikirim dan diterima oleh perangkat jaringan.  Karena kebanyakan aktifitas saat ini sudah berlangsung secara *online*, boleh dibilang semua aktifitas digital tercerminkan oleh paket jaringan yang keluar masuk.  Sebagai contoh, bila terjadi serangan *cyber* atau upaya pencurian data secara *remote*, informasi data apa yang bocor dan dikirim kemana dapat dijumpai di paket jaringan yang keluar masuk pada saat insiden terjadi...
---
...
```shell
stenoread 'after 24h ago' -w rekaman_harian.pcap
```
...
```shell
stenoread 'after 2021-02-25T20:00:00+07:00 and before 2021-02-25T21:00:00+07:00' -w rekaman.pcap
```
...
![Tampilan Protocol Hierarchy di Wireshark](../../../assets/gambar_20220226_001.png)
...
![Konfigurasi Jaringan Internal Network Di VirtualBox](../../../assets/gambar_20220226_002.png)
...
```shell
scp rekaman_harian.pcap user@192.168.56.3:~/rekaman_harian.pcap
...
sudo so-import-pcap rekaman_harian.pcap
```
...
![Tampilan SOC](../../../assets/gambar_20220226_003.png)
...
```shell
sudo mv rekaman_harian.pcap /opt/so/samples/rekaman_harian.pcap
sudo so-tcpreplay /opt/so/samples/rekaman_harian.pcap
```
...
```shell
sudo ip link set eth0 mtu 65535
sudo ip link set bond0 mtu 65535
```
